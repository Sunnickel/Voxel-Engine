...
impl Plugin for PlayerPlugin {
    fn build(&self, app: &mut App) {
        app.add_message::<MovementAction>()
            .add_systems(PostStartup, spawn_player.after(setup_chunk))
            .add_systems(
                Update,
                (
                    keyboard_input,
...
fn spawn_player(mut commands: Commands, spawn_point: ResMut<SpawnPoint>) {
    ...
    commands
        .spawn((
            ...
            Visibility::default(),
            CharacterControllerBundle::new(Collider::capsule(0.4, 2.0), Vector::NEG_Y * 9.81 * 2.0)
                .with_movement(50.0, 0.92, 7.0, (30.0 as Scalar).to_radians()),
        ))
        .with_children(|p| {
            p.spawn((Camera3d::default(),));
        });
}
